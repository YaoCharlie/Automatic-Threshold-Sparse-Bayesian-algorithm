...
%    指定推断的主要参数的结构:
%	.Value		Vector of weight values权值向量
%	.Relevant	Vector of corresponding indices of relevant基矩阵相关列对应指标向量(升序排序)
%				columns of BASIS matrix (sorted ascending)
%
%	HYPERPARAMETER	Structure specifying inferred hyperparameters:指定推断超参数的结构:
% 
%	.Alpha			Vector of weight precision values权重向量的精度值
%	.beta			Noise precision (Gaussian likelihood case)噪声精度(高斯似然情况)
% 
%	DIAGNOSTIC	Structure containing various diagnostics:包含各种诊断的结构:
% 
%	.Gamma		Vector of "well-determined" factors [0,1] for
%				relevant weights 相关权重的“确定”因子向量[0,1]
%	.Likelihood	Vector of evolving log-marginal-likelihood演化对数边际似然向量
%	.iterations	Number of iterations run迭代运行次数
%	.S_Factor	Vector of S ("Sparsity") values for relevant weights相关权重的S(“稀疏性”)值的向量
%	.Q_Factor	Vector of Q ("Quality") values for relevant weights相关权重的Q(“质量”)值向量
% 
% INPUT ARGUMENTS:
% 
%	LIKELIHOOD	String comprising one of 'Gaussian', 'Bernoulli' or 'Poisson'
%
%	BASIS		NxM matrix of basis vectors (one column per basis function)基向量的NxM矩阵(每个基函数一列)
%
%	TARGETS		N-vector with target output values有目标输出值的n向量
% 
%	OPTIONS		User-specified settings via SB2_USEROPTIONS [Optional]通过SB2_USEROPTIONS[可选]用户指定的设置
% 
%	SETTINGS	Initialisation of main parameter values via
%				SB2_PARAMETERSETTINGS [Optional]通过SB2_PARAMETERSETTINGS初始化主参数值[可选]
%
% NOTES: 
%
...
% LIKELIHOOD function, SPARSEBAYES will optimise优化 the log-marginal-likelihood
% of the corresponding sparse Bayesian model and should return (given
% reasonable choice of basis给予合理的基础项选择) a sparse vector of model parameters.
% 
% OPTIONS and SETTINGS arguments may be omitted省略了, and if so, will assume
% sensible default values.将假定合理的默认值。
% 
% SEE ALSO:
%
...
%   可用的用户定义选项，如要运行的迭代次数、诊断输出级别等。
%	SB2_PARAMETERSETTINGS: facility to change default initial values for
%	parameters and hyperparameters.
%   用于更改参数和超参数的默认初始值的工具。
%	SB2_CONTROLSETTINGS: hard-wired internal algorithm settings.硬连线内部算法设置。
%
